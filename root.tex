...
Recall@N (ou RecallRate@N ou R@N) é uma métrica que calcula a razão entre o número de imagens de busca com uma correspondência correta dentro das top N imagens recuperadas e todas as imagens de busca~\cite{vpr_bench, selavpr}, como mostrado na equação abaixo,

$$
R@N = \frac{T_p}{N_q}
$$

Em que $T_p$ são as imagens de busca corretas e $N_q = T_p + F_p + F_N + T_N$ são todas as imagens de busca. Essa definição é a mesma de acurácia no contexto de aprendizagem de máquina e, quando o recall é igual a $100\%$, $R@N$ assumi a mesma forma da equação de precisão. Essa métrica tem origem de \textit{information retrieval}~{if1}, usada em~\cite{ann1}.
